...
import { PlusCircle, User, Phone, Tags, Link2, Wallet, DollarSign, ArrowRightLeft, AlignLeft } from 'lucide-react'
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle, DialogTrigger } from '@/components/ui/dialog'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select'
import { createSharesBatch } from './actions'
import { toast } from 'sonner'

export function BatchAddShareDialog({ campaignId, defaultInternationalPrice, defaultInternationalSalePriceTl, fixedUsdRate, defaultRegion = 'YURTICI' }: { campaignId: string, defaultInternationalPrice?: number, defaultInternationalSalePriceTl?: number, fixedUsdRate?: number, defaultRegion?: string }) {
  ...
  async function onSubmit(formData: FormData) {
    ...
    try {
      await createSharesBatch(formData)
      toast.success("Toplu bağışçı kaydı başarıyla oluşturuldu.")
      setOpen(false)
    } catch (e: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger render={<Button className="bg-blue-600 hover:bg-blue-700 text-white font-semibold shadow-md cursor-pointer gap-1.5" />}>
        <PlusCircle className="h-4 w-4" /> Toplu Bağışçı Ekle
      </DialogTrigger>
      <DialogContent className="w-[95vw] sm:max-w-[650px] max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle>Toplu Bağışçı Girişi</DialogTitle>
          <DialogDescription>
            Aynı ödeme ve referans detaylarına sahip çok sayıda bağışçıyı tek seferde alt alta ekleyin.
          </DialogDescription>
        </DialogHeader>
        <form action={onSubmit} className="flex-1 overflow-y-auto px-1">
            <div className="grid gap-6 py-4">
              
              {/* Bölüm 1: Bağışçı Listesi */}
              <div className="space-y-4">
                <div className="flex items-center gap-2 border-b pb-2">
                  <AlignLeft className="w-5 h-5 text-emerald-600" />
                  <h3 className="font-semibold text-slate-800">Bağışçı İsim Listesi</h3>
                </div>
                <div className="grid gap-4">
                  <div className="grid gap-2">
                    <Label htmlFor="donor_names_raw" className="text-slate-600 font-semibold flex justify-between">
                      <span>Bağışçı Adı Soyadı Listesi (Her satıra bir isim)</span>
                      <span className="text-xs text-slate-400 font-normal">Excel veya WhatsApp'tan kopyalayabilirsiniz</span>
                    </Label>
                    <textarea 
                      id="donor_names_raw" 
                      name="donor_names_raw" 
                      rows={6}
                      required
                      placeholder="Örn:&#10;Ahmet Yılmaz&#10;Mehmet Demir&#10;Ayşe Kaya" 
                      className="w-full rounded-md border border-input bg-slate-50/50 px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                    />
                  </div>
                  <div className="grid gap-2">
                    <Label htmlFor="donor_phone" className="text-slate-600">Ortak Telefon Numarası (Tümü için geçerli - Opsiyonel)</Label>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="grid gap-2 p-3 bg-blue-50/40 rounded-md border border-blue-100 border-dashed justify-center flex flex-col">
                    <div className="flex items-center gap-2 mb-1">
                      <Link2 className="w-4 h-4 text-blue-500" />
                      <Label htmlFor="reference_name" className="text-blue-800 font-medium whitespace-nowrap">Referans Grubu / Ortak Referans</Label>
                    </div>
                    <Input id="reference_name" name="reference_name" placeholder="Örn: Esenyurt Temsilciliği" className="bg-white border-blue-200 focus-visible:ring-blue-300 font-semibold" />
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 border-b pb-2">
                  ...
                  <h3 className="font-semibold text-slate-800">Ortak Finansal Tahsilat (Her bir kişi için ayrı ayrı)</h3>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="grid gap-2 p-3 bg-emerald-50/50 border border-emerald-100 rounded-md">
                    <Label htmlFor="sale_price" className="text-emerald-800">Kişi Başı Satış Tutarı (Hisse Bedeli)</Label>
                    <div className="relative">
                  ...
                  <div className="grid gap-2 p-3 bg-blue-50/50 border border-blue-100 rounded-md">
                    <Label htmlFor="initial_payment" className="text-blue-800">Kişi Başı Alınan Peşinat</Label>
                    <div className="relative">
          ...
          <div className="sticky bottom-0 bg-white pt-4 pb-4 mt-2 border-t flex justify-end gap-3 px-1 flex-shrink-0">
            <Button type="button" variant="outline" onClick={() => setOpen(false)} disabled={loading}>
                İptal
            </Button>
            <Button type="submit" className="shadow-md px-8 bg-blue-600 hover:bg-blue-700 text-white" disabled={loading}>
              {loading ? 'Ekleniyor...' : 'Bağışçıları Toplu Ekle'}
            </Button>
          </div>
        </form>
      </DialogContent>
